...
float4 main(float2 uv : TEXCOORD0) : COLOR
{
    ...
    bool EnsureResources(IDirect3DDevice9* dev, UINT bbW, UINT bbH)
    {
        ...
        if (rw == g_rw && rh == g_rh && g_scene.surf && g_ping.surf && g_pong.surf && g_sb)
            return true;
    ...
    void SunViewToWorld()
    {
        ...
        if (dot < 0.99939f && g_sunLogs < 200)
        {
            ...
            SYSTEMTIME t;
            GetLocalTime(&t);
            Log("rays: sun from the sky: azimuth %.1f, elevation %.1f (dir %.3f %.3f %.3f) at %02d:%02d:%02d",
    ...
    bool SunOnScreen(SunScreen& s, float aspect)
    {
        ...
        // A direction, so w = 0: the camera's position (folded into each world matrix by this client,
        // see comfygrass's README) drops out and only the view rotation matters.
        ...
        if (r.sunMode == 2)
        {
            ...
            if (!g_haveSun)
                return false;
            dir[0] = g_sunDir[0]; dir[1] = g_sunDir[1]; dir[2] = g_sunDir[2];
        }
        else
        {
            const float az = r.azimuth * 0.01745329f, el = r.elevation * 0.01745329f;
            dir[0] = cosf(el) * cosf(az); dir[1] = cosf(el) * sinf(az); dir[2] = sinf(el);
        }

        float vs[4], clip[4];
        Transform4(dir, g_view, vs);
    ...
    void Run(IDirect3DDevice9* dev, IDirect3DSurface9* bb, UINT bbW, UINT bbH, const SunScreen& sun)
    {
        ...
        d->SetSamplerState(dev, 0, D3DSAMP_MAGFILTER, D3DTEXF_POINT);
        {
            IDirect3DTexture9* src = g_scene.tex;
        ...
        Pass(dev, g_scene.tex, g_ping.surf, g_rw, g_rh, g_psMask, maskC, 2);
        ...
        // --- composite ----------------------------------------------------------------------------
        // debugView 1 shows the mask and 2 the rays, each replacing the frame, for tuning.
        const float exposure = (r.strength * 0.01f) * r.maxExposure * sun.fade;
...
namespace
{
    ...
    void RunIfWanted(IDirect3DDevice9* dev)
    {
    const RaysSettings& r = g_cfg.rays;
...
bool RaysCamera(D3DMATRIX& view, D3DMATRIX& proj)
{
    if (!g_haveView || !g_haveProj)
        return false;
    view = g_view;
...
void RaysSetSunView(const float v[3])
{
